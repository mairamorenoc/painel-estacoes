...
server <- function(input, output, session) {
  ...
  session$onSessionEnded(function() {
    DBI::dbDisconnect(con, shutdown = TRUE) ## Close DB conn on session ends
  }) ## OBS. Manter "shutdown = TRUE" por enquanto. Rever depois
  ...
  observeEvent(input$station, {
    ...
    dates_df <- DBI::dbGetQuery(
      con,
      paste0("SELECT DISTINCT DATE(time) as d FROM ", input$station, " ORDER BY d") ## DB query to get unique available dates (without time stamps) sorted chronologically
    ...
    sensors_df <- DBI::dbGetQuery(
      con,
      paste0("SELECT DISTINCT sensor FROM ", input$station, " ORDER BY sensor") ## gets sensor col from DB and return df order by sensor
    )
  ...
  output$sensor_plot <- renderPlotly({
    ...
    for (sid in unique(df$sensor)) {
      ...
      if (is.null(config)) config <- sensor_config[["default"]] ## use default plot config for missing/new sensors
        ...
        layout(
          shapes = list(
            list(type="rect",xref="paper",x0=0,x1=1,y0=0,y1=3,
                 fillcolor="green",opacity=0.1,line=list(width=0)), ## Baixo risco queimaduras
            list(type="rect",xref="paper",x0=0,x1=1,y0=3,y1=6,
                 fillcolor="yellow",opacity=0.1,line=list(width=0)), ## Risco moderado
            list(type="rect",xref="paper",x0=0,x1=1,y0=6,y1=8,
                 fillcolor="orange",opacity=0.1,line=list(width=0)), ## Alto risco
            list(type="rect",xref="paper",x0=0,x1=1,y0=8,y1=20,
                 fillcolor="red",opacity=0.1,line=list(width=0)) ## Risco muito alto/extremo
          )
        )
    }
